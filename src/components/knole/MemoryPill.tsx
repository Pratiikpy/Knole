...
export function MemoryPill({ label, receipts }: { label: string; receipts: Receipt[] }) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="size-1.5 rounded-full bg-tan animate-breathe" aria-hidden="true" />
        {label}
        <span className="text-tan" aria-hidden="true">
          ·
        </span>
      ...
      {open && (
        <div
          id={panelId}
          className="animate-fade-up absolute left-0 top-full z-30 mt-2 w-[320px] rounded-xl border border-rule bg-card p-4 shadow-[0_24px_60px_-30px_rgba(28,25,23,0.25)]"
        >
