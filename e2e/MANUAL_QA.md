...
- **Expect:** the card flips to "Minted — token #N", with a working **0G explorer** link; tapping
  ...
  This step just confirms the _button → wallet → UI_ path in a real browser.
...
## 7. Mirror-reveal ceremony — needs a _freshly_ revealed mirror
...
_Generated as part of the production-level QA sweep. The automated suite covers every page (light +
dark), the deep feature flows, recall, on-chain mint, and the crisis safety net._
